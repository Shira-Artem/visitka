...
- Selected visual target: the second mobile concept, "Маршрут заказа" (`exec-dc505202-901c-4362-89ed-c038fce8cab9.png`).
- Preserve the desktop redesign's proposition and visual system: the entire product lives on phones inside Telegram or MAX; there is no PC product.
- The first screen must explain one order moving through Guest -> Cashier -> Kitchen -> Director, with real product screens as the primary visual material.
- Build the full mobile marketing page, not only the hero. Adapt the existing product screens and logic wherever possible.
- The cat is a small brand accent only; it must not replace the product story.
